...
3. **MCP token management UI.** *(Resolved & shipped.)* The token model is now **per-user, membership-capped** (`user_tokens`; one token spans all the user's projects — CANONICAL §6). Issue / list / revoke (copy-once) live in a personal MCP/API-token panel on the Instance Settings page (`/settings`), alongside an MCP connection guide; the MCP tools take a `site` (handle) selector. Tokens carry `scopes ⊆ {read,write,publish}` + optional expiry; effective scope per site = `token.scopes ∩ role`.
